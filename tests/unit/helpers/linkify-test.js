...
test('it should turn a ip address into a link', function (assert) {
  var result = linkify([
    'My link: https://62.123.123.123/test and some more text',
  ])
    .toString()
    .trim();
  assert.equal(
...
test('it should shorten a url by specified url length and adds 3 dots to the end in long url only', function (assert) {
  const longUrl =
    'https://guides.emberjs.com/v2.5.0/templates/writing-helpers/';
...
test('it should turn a space delimited list of urls into seperate links', function (assert) {
  var result = linkify([
    'My link: http://google.com http://bing.com www.altavista.com',
  ])
    .toString()
    .trim();
  assert.equal(
